...
import { Search, Plus, Edit2, Trash2, Package, Store, MoreVertical, Download, Upload } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background">
      {/* Header Optimized for Android 360px */}
      <header className="px-4 pt-4 pb-2 bg-white/95 backdrop-blur-md sticky top-0 z-10 border-b flex flex-col gap-2">
        ...
        {/* Row 3: Horizontal Scroll Category */}
        <div className="w-full -mx-4 px-4 overflow-x-auto no-scrollbar touch-pan-x flex">
          <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="w-full">
            <TabsList className="h-auto bg-transparent p-0 justify-start flex flex-nowrap w-max gap-1 pb-1">
              {categories.map(cat => (
                <TabsTrigger 
                  key={cat} 
                  value={cat}
